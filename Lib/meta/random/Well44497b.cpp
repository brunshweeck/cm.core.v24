...
#include <meta/random/Well44497b.h>

namespace core {
    namespace random {
        atomic::AtomicLong Well44497b::DEFAULT_GENERATOR = atomic::AtomicLong(RandomSupport::initialSeed());

        Well44497b::Well44497b() {
            setSeed(DEFAULT_GENERATOR.getAndAdd(RandomSupport::GOLDEN_RATIO_64));
        }

        Well44497b::Well44497b(gint seed) {
            setSeed(seed);
        }

        Well44497b::Well44497b(IntArray const &seed) {
            setSeed(seed);
        }

        Well44497b::Well44497b(glong seed) {
            setSeed(seed);
        }

        gint Well44497b::nextInt() const {
            return next(32);
        }

        gbool Well44497b::nextBoolean() const {
            return next(1) != 0;
        }

        gfloat Well44497b::nextFloat() const {
            return CORE_CAST(gfloat, next(23)) * 0x1.0p-23F;
        }

        gdouble Well44497b::nextDouble() const {
            const glong high = CORE_CAST(glong, next(26)) << 26;
            const gint low = next(26);
            return CORE_CAST(gdouble, high | low) * 0x1.0p-52;
        }

        glong Well44497b::nextLong() const {
            const glong high = CORE_CAST(glong, next(32)) << 32;
            const glong low = CORE_CAST(glong, next(32)) & 0xffffffffL;
            return high | low;
        }

        gdouble Well44497b::nextGaussian() const {
        ...
        void Well44497b::setSeed(gint seed) {
            setSeed(IntArray::of(seed));
        }

        void Well44497b::setSeed(IntArray const &seed) {
            if (seed.isEmpty()) {
                setSeed(DEFAULT_GENERATOR.getAndAdd(RandomSupport::GOLDEN_RATIO_64));
                return;
            }
        ...
        void Well44497b::setSeed(glong seed) {
            setSeed(IntArray::of(CORE_CAST(gint, (seed + 0u) >> 32), CORE_CAST(gint, seed & 0xffffffffl)));
        }

        gbool Well44497b::isDeprecated() const {
            return false;
        }

        RandomGeneratorProperties Well44497b::properties() const {
            return RandomGeneratorProperties("Well44497b"_S, "WELL"_S, 512, 1, 0);
        }

        gint Well44497b::next(gint bits) const {
            ...
            int z4             = z0 ^ (z1 ^ ((z1 + 0u) >> 20)) ^ z2Second ^ z3;
