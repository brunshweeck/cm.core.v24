...
namespace core {
    namespace regex {
        gbool Pattern::IndicConjunctBreak::isLinker(gint cp) {
            return
                    cp == 0x094D ||
                    cp == 0x09CD ||
                    cp == 0x0ACD ||
        ...
        gbool Pattern::IndicConjunctBreak::isExtend(gint cp) {
            return
                    (cp >= 0x0300 && cp <= 0x034E) ||
                    (cp >= 0x0350 && cp <= 0x036F) ||
                    (cp >= 0x0483 && cp <= 0x0487) ||
        ...
        gbool Pattern::IndicConjunctBreak::isConsonant(gint cp) {
