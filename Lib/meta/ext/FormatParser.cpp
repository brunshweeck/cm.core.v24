  ...
  namespace ext {
    ...
    gbool SpecialFragment::apply(Formatter& fmt, XString& sb, gdouble const& value,
      ...
      if (c == Conversion::SCIENTIFIC) {
        ...
        gint prec = precision;
        if (precision == -1)
          // assume that we want all the digits
          prec = 0;
        else if (precision == 0)
          prec = 1;
      ...
      try {
        ...
        switch (c) {
          case DateTime::HOUR_OF_DAY_0: { // 'H' (00 - 23)
            glong i = t.getLong(TemporalField::HOUR_OF_DAY);
          ...
          case DateTime::HOUR_0: { // 'I' (01 - 12)
            glong i = t.getLong(TemporalField::CLOCK_HOUR_OF_AMPM);
            sb.append(magnitude(fmt, xs, i, Flags::ZERO_PAD, 2));
            break;
          }
          ...
          case DateTime::MINUTE: { // 'M' (00 - 59)
            glong i = t.getLong(TemporalField::MINUTE_OF_HOUR);
            sb.append(magnitude(fmt, xs, i, Flags::ZERO_PAD, 2));
            break;
          }
          case DateTime::NANOSECOND: { // 'N' (000000000 - 999999999)
            glong i;
            try {
              i = t.getLong(TemporalField::NANO_OF_SECOND);
            } catch (TemporalException const& _) {
              i = t.getLong(TemporalField::MILLI_OF_SECOND) * 1000000;
            }
            sb.append(magnitude(fmt, xs, i, Flags::ZERO_PAD, 9));
            break;
          }
          case DateTime::MILLISECOND: { // 'L' (000 - 999)
            glong i = t.getLong(TemporalField::MILLI_OF_SECOND);
            sb.append(magnitude(fmt, xs, i, Flags::ZERO_PAD, 3));
            break;
          }
          case DateTime::MILLISECOND_SINCE_EPOCH: { // 'Q' (0 - 99...?)
            glong i = t.getLong(TemporalField::INSTANT_SECONDS) * 1000L +
                t.getLong(TemporalField::MILLI_OF_SECOND);
          ...
          case DateTime::AM_PM: { // 'p' (am or pm)
            ...
            const String& s = ampm[t.getLong(TemporalField::AMPM_OF_DAY)];
            sb.append(s.toLowerCase(Locale::getDefault(Locale::FORMAT)));
            break;
          }
          case DateTime::SECONDS_SINCE_EPOCH: { // 's' (0 - 99...?)
            glong i = t.getLong(TemporalField::INSTANT_SECONDS);
            sb.append(magnitude(fmt, xs, i, Flags::NONE, w));
            break;
          }
          case DateTime::SECOND: { // 'S' (00 - 60 - leap second)
            glong i = t.getLong(TemporalField::SECOND_OF_MINUTE);
            sb.append(magnitude(fmt, xs, i, Flags::ZERO_PAD, 2));
            break;
          }
          case DateTime::ZONE_NUMERIC: { // 'z' ({-|+}####) - ls minus?
            int i = t.getLong(TemporalField::OFFSET_SECONDS);
            gbool neg = i < 0;
            sb.append(neg ? '-' : '+');
            if (neg)
              i = -i;
          ...
          case DateTime::ZONE: { // 'Z' (symbol)
            ...
            ZoneId& zid = opt.get();
            if (!Class<ZoneOffset>::hasInstance(zid) && t.isSupported(TemporalField::INSTANT_SECONDS)) {
              // glong instant = t.getLong(TemporalField::INSTANT_SECONDS);
              // sb.append(TimeZone::of(zid.getId())
              //                   .displayName(zid.getRules()
              //                   .isDaylightSavings(LocalDateTime::ofEpochSecond(instant/1000, 0)),
              //              FormatStyle::SHORT, fmt.locale));
              // break;
            }
            sb.append(zid.getId());
          ...
          case DateTime::NAME_OF_DAY: { // 'A'
            int i = t.getLong(TemporalField::DAY_OF_WEEK) % 7 + 1;
            i -= 1; // for index
          ...
          case DateTime::NAME_OF_MONTH: { // 'B'
            int i = t.getLong(TemporalField::MONTH_OF_YEAR) - 1;
          ...
          case DateTime::YEAR_4: { // 'Y' (0000 - 9999)
            glong i = t.getLong(TemporalField::YEAR_OF_ERA);
            int size = 2;
            switch (c) {
          ...
          case DateTime::DAY_OF_MONTH: { // 'e' (1 - 31) -- like d
            glong i = t.getLong(TemporalField::DAY_OF_MONTH);
          ...
          case DateTime::DAY_OF_YEAR: { // 'j' (001 - 366)
            glong i = t.getLong(TemporalField::DAY_OF_YEAR);
            sb.append(magnitude(fmt, xs, i, Flags::ZERO_PAD, 3));
            break;
          }
          case DateTime::MONTH: { // 'm' (01 - 12)
            glong i = t.getLong(TemporalField::MONTH_OF_YEAR);
            sb.append(magnitude(fmt, xs, i, Flags::ZERO_PAD, 2));
            break;
          }
    ...
    void FormatParser::parse(String const& format) {
      ...
      while (i < max) {
        ...
        if (Conversion::isValid(c)) {
          ...
          if (matcher.isEmpty()) {
            static String const FORMAT_SPECIFIER =
                R"(%(\d+\$)?([-#+ 0,(\<]*)?(\d+)?(\.\d+)?([tT])?([a-zA-Z%]))"_Sl;
