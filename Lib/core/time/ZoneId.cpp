...
namespace core {
    ...
    namespace time {
        ...
        ZoneId const& ZoneId::ofOffset(String const& prefix, ZoneOffset const& offset) {
            ...
            if (prefix.isEmpty()) {
                CACHE_IDS.putIfAbsent(offset.getId(), ZoneOffset(offset.totalSeconds()));
        ...
        ZoneId const& ZoneId::from(Temporal const& temporal) {
            Optional<ZoneId> obj = (Optional<ZoneId>) temporal.query(TemporalQuery::ZONE);
            if (obj.isEmpty()) {
        ...
        static String queryDisplayName(gint region, Temporal::FormatStyle style, gint LID, gint SID, gint RID) {
        ...
        static String queryGenericDisplayName(gint tz, Temporal::FormatStyle style, gint LID, gint SID, gint RID) {
            gbool L = LID >= 0, S = SID >= 0, R = RID;
            Cldr& cldr = Cldr::forLocale(LID, RID, SID, -1);
            S = cldr.hasScript();
            R = cldr.hasRegion();
        ...
        Optional<> ZoneId::query(TemporalQuery const& query) const {
        ...
        ZoneId const& ZoneId::of(String const& zoneId, gbool checkAvailable) {
...

            {
                ...
                if (zId != null) {
                    if (Class<ZoneOffset>::hasInstance(zId)) {
                        ZoneOffset &zOffset = CORE_XCAST(ZoneOffset, zId);
                        ZoneOffset offset = ZoneOffset(zOffset.totalSeconds()); // To ReCreate ID
                        if (offset.getId() != CORE_XCAST(ZoneOffset, zId).getId()) {
                            // Offset has been modified (resetting offset)
                            zOffset = ZoneOffset::of(zOffset.getId());
                        }
                    }
        ...
        ZoneRules ZoneId::regionToRules(String const& region) {
            ...
            Array<ZoneOffset> standardOffsets = Array<ZoneOffset>(rules[1].length());
            for (gint i = 0; i < rules[1].length(); i++) {
                standardOffsets.set(i, ZoneOffset(rules[1][i]));
            }
            const LongArray& savingsInstantTransitions = rules[2];
            Array<ZoneOffset> wallOffsets = Array<ZoneOffset>(rules[3].length());
            for (gint i = 0; i < rules[3].length(); i++) {
                wallOffsets.set(i, ZoneOffset(rules[3][i]));
            }
            Array<ZoneOffsetTransitionRule> lastRules = Array<ZoneOffsetTransitionRule>(transitionRules.length());
            for (gint i = 0; i < transitionRules.length(); i++) {
                CORE_ASSERT(transitionRules[i].length() == 9);
                lastRules.set(i, ZoneOffsetTransitionRule(
                                  (LocalDate::Month) transitionRules[i][0],
                                  transitionRules[i][1],
                                  (LocalDate::DayOfWeek) transitionRules[i][2],
                                  ...
                                  ZoneOffset(transitionRules[i][6]),
                                  ZoneOffset(transitionRules[i][7]),
                                  ZoneOffset(transitionRules[i][8])
                              ));
            }
            return ZoneRules(standardTransitions,
                             standardOffsets,
