...
namespace core {
  ...
  CLASS_ANALYZING:
    ...
    if (declaringClass.indexOf('~') >= 0)
      IllegalArgumentException("Invalid class name for input: "_S + declaringClass).throws($ftrace());
    if (declaringClass.indexOf('.') >= 0)
  ...
  GENERALIZATION: {
      ...
      while ((i = s.indexOf('<', j)) >= j) {
        ...
        } else if (i + 1 < s.length() && !op && (s.charAt(i + 1) == '(' || s.charAt(i + 1) == '=')) {
          // operator <(.*)...
          gint k = s.lastIndexOf("operator <"_S, i + 1);
          if (k < 0)
            k = s.lastIndexOf("operator<"_Sl, i + 1);
        ...
        if (k == s.length()) {
          IllegalArgumentException("Malformed name after index "_S + i + " for input: " + mc)
              .throws($ftrace());
          return { };
        }
        r += s.subString(j, i);
  ...
  ANALYZING: {
      ...
      for (gint i = 0; i < s.length(); i++) {
        gchar c = s.charAt(i);
        if (c > 0x7F) {
          IllegalArgumentException("Non-ASCII name at index "_S + i + " for input: " + s)
              .throws($ftrace());
          return String();
        }
        if (Character::isLetterOrDigit(c) || c == '$' || c == '_') {
          if (Character::isDigit(c) && !letter) {
            IllegalArgumentException("Malformed name at index "_S + i + " for input: " + s)
                .throws($ftrace());
          }
          letter = true;
          separator = false;
        ...
        if (c == '(' || c == ')' || // For operator ()
          c == '[' || c == ']' || // For operator []
          c == '+' || // For operator +, operator +=, operator ++
          c == '-' || // For operator -, operator -=, operator --, operator ->
          c == '*' || // For operator *, operator *=
          c == '/' || // For operator /, operator /=
          c == '%' || // For operator %, operator %=
          c == '^' || // For operator ^, operator ^=
          c == '=' || // For operator =, operator ==
          c == '!' || // For operator !, operator !=
          c == '<' || // For operator <, operator <=, operator <<, operator <<=, operator <=> (C++20)
          c == '>' || // For operator >, operator >=, operator >>, operator >>=, operator <=> (C++20)
          c == '&' || // For operator &, operator &=, operator &&
          c == '|' || // For operator |, operator |=, operator ||
