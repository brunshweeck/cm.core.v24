...
namespace core {
  ...
  public:
     ...
     * @param z the complex value
     */
    template <class Z, ClassOf(1)::OnlyIf<Class<Complex>::isSame<typename Class<Z>::Object>()>  = 1>
    CORE_IMPLICIT Complex(Z const& z): Complex(
      initLiteral((glong) &z, Class<Z>::size(), Class<Z>::template isSameToOne<_Fcomplex, _Dcomplex, _Lcomplex>())) {
