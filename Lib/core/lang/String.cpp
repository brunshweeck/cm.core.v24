...
#include <meta/lang/CharacterDataLatin1.h>
#include <meta/lang/StringUtils.h>
...
namespace core {
  ...
  String::String(ByteArray const& bytes, Charset const& charset): String() {
    ...
    if (Class<ArrayDecoder>::hasInstance(dec)) {
      ArrayDecoder& ad = CORE_XCAST(ArrayDecoder, dec);
      // ascii
      ...
      dec.onMalformedInput(Charset::REPLACE).onMalformedInput(Charset::REPLACE);
    ...
    gint en = (gint) (length * dec.maxCharsPerByte());
    dec.onMalformedInput(Charset::REPLACE).onUnmappableCharacter(Charset::REPLACE);
    CharArray dst = CharArray(en);
    gint cnt = 0;
    try {
    ...
    } catch (CharacterCodingException const& ex) {
  ...
  ByteArray String::toBytes(String const& charsetName) const {
    try {
      return toBytes(Charset::forName(charsetName));
    } catch (UnsupportedCharsetException
      const& _) {
      UnsupportedCharsetException(charsetName).throws($ftrace());
    } catch (
      IllegalCharsetNameException const& _) {
      UnsupportedCharsetException(charsetName).throws($ftrace());
    } catch (Throwable const& ex) { ex.throws($ftrace()); }
  }

  ByteArray String::toBytes(Charset const& charset) const {
    if (&charset == &Charset::UTF_8) {
      return coder == LATIN1
    ...
    try {
      ...
      if (Class<ArrayEncoder>::hasInstance(enc)) {
        ArrayEncoder& ae = CORE_XCAST(ArrayEncoder, enc);
      ...
      if (len == 0)
        return dst;
      enc.onMalformedInput(Charset::REPLACE).onUnmappableCharacter(Charset::REPLACE);
