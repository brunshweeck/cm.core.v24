...
namespace core {
    ...
    namespace concurrent {
        ...
        glong QueuedSynchronizer::ConditionObject::awaitNanos(glong nanosTimeout) {
            ...
            while (!canReacquire(node)) {
                if ((interrupted |= Thread::interrupted()) ||
                    (nanos = nanos - chrono.elapsedTime(Chronometer::ChronoUnit::NANOS)) <= 0L) {
                    if ((cancelled = (node->getAndUnsetStatus(COND) & COND) != 0))
                        break;
                } else
            ...
            if (cancelled) {
                ...
                Thread::currentThread().interrupt();
            glong remaining = nanos - chrono.elapsedTime(Chronometer::ChronoUnit::NANOS); // avoid overflow
            return (remaining <= nanosTimeout) ? remaining : Long::MIN_VALUE;
        }

        gbool QueuedSynchronizer::ConditionObject::awaitUntil(const time::LocalDateTime& deadline) {
            glong abstime = deadline.toEpochSecond() * 1000 + deadline.toLocalTime().nano() / 1000000;
            if (Thread::interrupted())
                InterruptedException().throws($ftrace());
            COND_NODE node = newConditionNode();
            if (!node)
            ...
            chrono.start();
            while (!canReacquire(node)) {
                time::LocalDateTime dt = time::LocalDateTime::now();
                glong current = dt.toEpochSecond() * 1000 + dt.toLocalTime().nano() / 1000000;
        ...
        gbool QueuedSynchronizer::ConditionObject::await(glong time, Temporal::ChronoUnit unit) {
            if (Thread::interrupted())
                InterruptedException().throws($ftrace());
            COND_NODE node = newConditionNode();
            if (!node)
        ...
        gint QueuedSynchronizer::acquire(NODE node, gint arg, gbool shared,
            ...
            for (;;) {
                ...
                if (!((t = tail))) {
                    ...
                    else if ((nanos = time - chrono.elapsedTime(Chronometer::ChronoUnit::NANOS)) > 0L)
                        LockSupport::parkNanos(*this, nanos);
                    else
                        break;
                    node->clearStatus();
